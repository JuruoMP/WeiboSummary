...
# data_path = os.path.join(file_path, "toy")
...
def train():
  ...
  config = tf.ConfigProto(device_count={"CPU": 12}, # limit to 4 CPU usage
                   inter_op_parallelism_threads=3, 
                   intra_op_parallelism_threads=4) # n threads parallel for ops
  ...
  with tf.Session(config = config) as sess:
    ...
    print('len(dev_set) = %d' % sum([len(bucket) for bucket in dev_set]))  # print total dev set
