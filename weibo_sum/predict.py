...
def generate_summary(input_dir, reference_dir, summary_dir):
  ...
  for line in input_file:
    ...
    sentences.append(line.replace("\n",""))   # list of 'str', convert 'unicode' to 'str'
  ...
  with tf.Session() as sess:
    ...
    for i in range(len(sentences)):
      ...
      # token_ids = data_utils.sentence_to_token_ids(tf.compat.as_bytes(sentence), vocab)
      token_ids = data_utils.sentence_to_token_ids(tf.compat.as_str(sentence), vocab)
      avaliable_buckets = [b for b in xrange(len(buckets)) if buckets[b][0] > len(token_ids)]
      if avaliable_buckets:
        bucket_id = min(avaliable_buckets)
      else:
        continue
      # Get a 1-element batch to feed the sentence to the model.
      encoder_inputs, decoder_inputs, target_weights = model.get_batch(
